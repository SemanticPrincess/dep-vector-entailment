...
function [ cost, grad, trainingError, confusion ] = ComputeFullDepCostAndGrad( theta, decoder, data, hyperParams, ~ )
...
if nargout > 1
    parfor i = 1:N
        [localCost, localGrad, localPred] = ...
            ComputeDepCostAndGrad(theta, decoder, data(i), hyperParams);
        ...
        if (~localCorrect) && (argout > 2) && hyperParams.showExamples
            disp([num2str(localCost), ' for: ', data(i).leftTree.getText(), ' ', ...
